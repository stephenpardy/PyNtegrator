...
def corner(xs, weights=None, labels=None, extents=None, truths=None,
    ...
    for i, x in enumerate(xs):
        ...
        n, b, p = ax.hist(x, weights=weights, bins=kwargs.get("bins", 50),
                          range=extents[i],color=mpl.colors.hex2color("#ABCEEA"), histtype="stepfilled")
        if truths is not None:
            ax.axvline(truths[i], color=truth_color, linewidth=2)

#new for median:
        ax.axvline(np.median(x), color='r', linewidth=2)
        ...
        for j, y in enumerate(xs):
            ...
            if truths is not None:
                ...
                ax.axvline(truths[j], color=truth_color, linewidth=2)
                ax.axhline(truths[i], color=truth_color, linewidth=2)
