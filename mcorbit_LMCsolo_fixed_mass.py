...
def Serial_orbit(name):
    pool = get_pool(mpi=False, threads=16)
...
def bad_parameters(p):
    init_vel = np.array([-77.0, -224.0, 227.0])
    sigma_errors = [8, 14, 18]
...
def loglikelihood(p):
    ...
    ln_likelihood = pyorbits.test_stream_nosmc(params, stream_data, False)
