...
def init_walker(params, lmc_vel=[0, 0, 0], smc_vel=[0, 0, 0]):
    fit = np.NINF
    while (fit < -2000):
        params[0] = lmc_vel[0] + 8*gauss(0.0, 1)
        params[1] = lmc_vel[1] + 14*gauss(0.0, 1)
        params[2] = lmc_vel[2] + 18*gauss(0.0, 1)
        params[3] = smc_vel[0] + 18*gauss(0.0, 1)
        params[4] = smc_vel[1] + 21*gauss(0.0, 1)
        params[5] = smc_vel[2] + 17*gauss(0.0, 1)
...
def Serial_orbit(name):
    ...
    # Use the pool to test initially
